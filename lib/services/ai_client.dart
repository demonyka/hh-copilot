...
class AiClient {
  ...
  Future<InterviewVerdict> classifyInterview(
    ...
    const system =
        'Ты СТРОГО классифицируешь сообщение работодателя соискателю на hh.ru.\n'
        'Верни interview=true ТОЛЬКО если работодатель делает конкретный шаг к '
        'собеседованию:\n'
        '- зовёт на собеседование/интервью/встречу (в т.ч. видеозвонок, Zoom/Meet);\n'
        '- предлагает СОЗВОНИТЬСЯ или просит телефон именно ДЛЯ ЗВОНКА;\n'
        '- назначает или спрашивает КОНКРЕТНОЕ ВРЕМЯ звонка/встречи '
        '(«когда удобно созвониться?», «завтра в 15:00?»);\n'
        '- делает оффер / предлагает выйти на работу.\n\n'
        'Во ВСЕХ остальных случаях interview=false, в частности:\n'
        '- сообщения от AI/бота-ассистента, авто-приветствия, «спасибо за отклик»;\n'
        '- «готовы рассмотреть вакансию?», «обсудим детали в чате?», «расскажите о себе», '
        'уточняющие вопросы про опыт/стек/зарплату — это переписка, НЕ собеседование;\n'
        '- просьба заполнить анкету/форму/тест, ссылки, реклама, отказ.\n'
        'Обычный чат и скрининг в переписке — это НЕ собеседование. Сомневаешься — false.\n'
        'Верни только JSON: {"interview": true|false, "kind": "звонок|встреча|оффер|", "summary": "коротко"}.';
    final user =
